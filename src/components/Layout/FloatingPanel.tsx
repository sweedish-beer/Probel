  ...
  const FloatingPanel: React.FC<FloatingPanelProps> = ({
    ...
    const handleMaximize = () => {
        if (isMaximized) {
            ...
            // Calculate size based on viewport, not window
            const parentElement = panelRef.current?.parentElement;
            const parentRect = parentElement?.getBoundingClientRect();
            if (parentRect) {
                setSize({
                    width: parentRect.width - 40,
                    height: parentRect.height - 40
                });
                setPosition({ x: 20, y: 20 });
            } else {
                // Fallback to window dimensions if parent not available
                setSize({
    ...
    return (
        <Paper
            ref={panelRef}
            elevation={activePanel ? 8 : 3}
            sx={{
                position: 'absolute',
                left: `${position.x}px`,
                top: `${position.y}px`,
                width: isMinimized ? 250 : `${size.width}px`,
                height: isMinimized ? 48 : `${size.height}px`,
                display: 'flex',
